...
var emscriptenMemoryProfiler = {
  ...
  bytesToPixelsRoundedDown(bytes) {
    return (bytes * emscriptenMemoryProfiler.canvas.width * emscriptenMemoryProfiler.canvas.height / (growMemViews(), 
    HEAP8).length) | 0;
  },
  ...
  bytesToPixelsRoundedUp(bytes) {
    return ((bytes * emscriptenMemoryProfiler.canvas.width * emscriptenMemoryProfiler.canvas.height + (growMemViews(), 
    HEAP8).length - 1) / (growMemViews(), HEAP8).length) | 0;
  },
  ...
  updateUi() {
    ...
    var width = (nBits((growMemViews(), HEAP8).length) + 3) / 4;
    // Pointer 'word width'
    var html = "Total HEAP size: " + self.formatBytes((growMemViews(), HEAP8).length) + ".";
    ...
    html += "<br />" + colorBar("#6699CC") + colorBar("#003366") + colorBar("#0000FF") + "DYNAMIC memory area used: " + self.formatBytes(self.totalMemoryAllocated) + " (" + (self.totalMemoryAllocated * 100 / ((growMemViews(), 
    HEAP8).length - heap_base)).toFixed(2) + "% of all dynamic memory and unallocated heap)";
    html += "<br />Free memory: " + colorBar("#70FF70") + "DYNAMIC: " + self.formatBytes(heap_end - heap_base - self.totalMemoryAllocated) + ", " + colorBar("#FFFFFF") + "Unallocated HEAP: " + self.formatBytes((growMemViews(), 
    HEAP8).length - heap_end) + " (" + (((growMemViews(), HEAP8).length - heap_base - self.totalMemoryAllocated) * 100 / ((growMemViews(), 
    HEAP8).length - heap_base)).toFixed(2) + "% of all dynamic memory and unallocated heap)";
...
var _emscripten_trace_report_memory_layout = () => {
  if (EmscriptenTrace.postEnabled) {
    var memory_layout = {
      ...
      "dynamic_top": _sbrk(0),
      "total_memory": (growMemViews(), HEAP8).length
    };
    var now = EmscriptenTrace.now();
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, __setitimer_js, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write ];
...
var _evaluate_by_search = Module["_evaluate_by_search"] = makeInvalidEarlyAccess("_evaluate_by_search");

var _new_game = Module["_new_game"] = makeInvalidEarlyAccess("_new_game");

var _best_move = Module["_best_move"] = makeInvalidEarlyAccess("_best_move");

var _init = Module["_init"] = makeInvalidEarlyAccess("_init");

var _destroy = Module["_destroy"] = makeInvalidEarlyAccess("_destroy");
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports["evaluate_by_search"] != "undefined", "missing Wasm export: evaluate_by_search");
  assert(typeof wasmExports["new_game"] != "undefined", "missing Wasm export: new_game");
  assert(typeof wasmExports["best_move"] != "undefined", "missing Wasm export: best_move");
  assert(typeof wasmExports["init"] != "undefined", "missing Wasm export: init");
  assert(typeof wasmExports["destroy"] != "undefined", "missing Wasm export: destroy");
  ...
  _evaluate_by_search = Module["_evaluate_by_search"] = createExportWrapper("evaluate_by_search", 1);
  _new_game = Module["_new_game"] = createExportWrapper("new_game", 0);
  _best_move = Module["_best_move"] = createExportWrapper("best_move", 0);
  _init = Module["_init"] = createExportWrapper("init", 0);
  _destroy = Module["_destroy"] = createExportWrapper("destroy", 0);
